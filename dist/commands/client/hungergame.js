...
const hungergames_1 = require("../../types/hungergames");
exports.command = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .setRequired(false))
        .addUserOption(option => option.setName('tributo1')
        .setDescription('Añade un tributo')
        .setRequired(false))
        .addUserOption(option => option.setName('tributo2')
        .setDescription('Añade otro tributo')
        .setRequired(false))
        .addUserOption(option => option.setName('tributo3')
        .setDescription('Añade otro tributo')
        .setRequired(false))
        .addUserOption(option => option.setName('tributo4')
        .setDescription('Añade otro tributo')
        .setRequired(false))),
    async execute(client, interaction) {
        ...
        if (interaction.options.getSubcommand() === "iniciar") {
            ...
            const rolMencionado = interaction.options.getRole('rol');
            ...
            const embedInicio = new discord_js_1.EmbedBuilder()
                .setColor("Orange")
            ...
            collector.on('collect', async (i) => {
                if (i.customId === 'registrar_tributo') {
                    ...
                    if (rolMencionado && i.member) {
                        if (i.member.roles instanceof discord_js_1.GuildMemberRoleManager) {
        ...
        async function iniciarRonda(client, interaction, tributos) {
            ...
            if (tributosVivos.length <= 0) {
                ...
                juegoEnCurso = false;
                const embedGanador = new discord_js_1.EmbedBuilder()
                    .setColor("Gold")
            ...
            const embedRonda = new discord_js_1.EmbedBuilder()
                .setColor("Blue")
            ...
            if (tributosVivos.length >= 2) {
                ...
                const ejecutarEventosConCooldown = async (eventos, index = 0) => {
                    if (index < eventos.length) {
                        await (0, hungergames_1.ejecutarEvento)(eventos[index].tributo1, eventos[index].tributo2, tributos, interaction);
                        ...
                        const embedVidas = new discord_js_1.EmbedBuilder()
                            .setColor("Purple")
